...
export function onMessageDeleted(socket: Socket | null, handler: (payload: {
    messageId: string;
    chatId: string;
    senderId?: string
}) => void) {
...
export async function sendTyping(socket: Socket | null, chatId: string, username: string | undefined): Promise<void> {
    if (!socket) return;
...
export function onTyping(socket: Socket | null, handler: (payload: {
    chatId: string;
    username: string;
    userId: string
}) => void) {
    ...
    const listener = (payload: unknown) => {
        try {
            const obj = payload as { chatId?: string; username?: string; userId?: string } | undefined;
