...
public class UploadChatAvatarStreamHandler : IRequestHandler<UploadChatAvatarStreamCommand, Result<string>>
{
    ...
    public UploadChatAvatarStreamHandler(
    ...
    public async Task<Result<string>> Handle(UploadChatAvatarStreamCommand request, CancellationToken cancellationToken)
    {
        if (request.File.Content is null || request.File.Content.Length == 0)
        ...
        if (request.File.ContentLength.HasValue && request.File.ContentLength.Value > _userDefaults.MaxAvatarSizeInBytes)
            return Result<string>.Failure($"File size exceeds the limit of {_userDefaults.MaxAvatarSizeInBytes} bytes.");

        try
        {
            // Stream-based save - no memory allocation for file content
